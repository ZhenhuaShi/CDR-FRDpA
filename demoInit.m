...
nIt = 1000;
Nbs = 64;
LN0={'Rand','initRand','PCAxy','initPCAxy','PCA','initPCA'};
...
% parfor r=1:nRepeats
    ...
    for s=1:length(datasets)
        ...
        for nF=nFs0
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','Rand','nF',nF,'Init','FCM','nMF',nRule,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs);
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','initRand','nF',nF,'Init','FCM','nMF',nRule,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs);
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','PCAxy','nF',nF,'Init','FCM','nMF',nRule,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs);
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','initPCAxy','nF',nF,'Init','FCM','nMF',nRule,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs);
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','PCA','nF',nF,'Init','FCM','nMF',nRule,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs);
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
...
save('demoInit.mat','MF','Powerball','DropRule','lr','l2','nIt','Nbs','RMSEtrain','RMSEtune','RMSEtest','times','BestF','BestmIter','thres','datasets','nAlgs','LN','LN0','nRepeats','nFs','nRs');
