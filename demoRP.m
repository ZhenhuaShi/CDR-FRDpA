...
Nbs = 64;
MLS = 1:10;
LN0={'DropRule=0.5','DropRule=1','Rule Pruning','Iterative RP', 'Adaptive RP','Iterative ARP'}; %CDR-FCM-RDpA
LN1={'RR'};
...
% parfor r=1:nRepeats
    ...
    for s=1:length(datasets)
        ...
        for nF=nFs0
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','CDR','nF',nF,'Init','FCM','nMF',nRule,'Opt','AdaBelief','Powerball',Powerball,'DropRule',1,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs);
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','CDR','nF',nF,'Init','FCM','nMF',nRule*2,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs,'RP',fix(nIt/2));
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','CDR','nF',nF,'Init','FCM','nMF',nRule*2,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs,'RP',fix([nIt/2, 2*nIt/3]));
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','CDR','nF',nF,'Init','FCM','nMF',nRule*2,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs,'RP',fix([0, nIt/2]));
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
            ...
            tic;
            id=id+1;
            [tmp,tmpt,~, ~, ~, ~, fB]=sugfis_mbgd(XTrain,yTrain,{XTune,XTest},{yTune,yTest},'MF',MF,'DR','CDR','nF',nF,'Init','FCM','nMF',nRule*2,'Opt','AdaBelief','Powerball',Powerball,'DropRule',DropRule,'lr',lr,'l2',l2,'nIt',nIt,'Nbs',Nbs,'RP',fix([0, nIt/2, 2*nIt/3]));
            if min(tmpt{1})<thres{r}(s,id)||~isfinite(thres{r}(s,id))
                [thres{r}(s,id),BestmIter{r}(s,id)]=min(tmpt{1});
                BestF{r}(s,id,1:nRule)=fB;
                [RMSEtrain{r}(s,id,:),RMSEtune{r}(s,id,:),RMSEtest{r}(s,id,:)]=deal(tmp,tmpt{1},tmpt{2});
...
save('demoRP.mat','MF','Powerball','DropRule','lr','l2','nIt','Nbs','RMSEtrain','RMSEtune','RMSEtest','times','BestF','BestmIter','thres','datasets','nAlgs','LN','LN0','LN1','nRepeats','nFs','nRs');
